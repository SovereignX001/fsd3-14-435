...
const removeProduct = async (pid) => {
  ...
  if (isFoundInCart) {
    cart.splice(cart.indexOf(isFoundInCart), 1);
    await saveCart(cart);
    console.log(`${isFoundInCart.name} removed from 🛒`);
  }
};
  ...
  do {
    ...
    switch (Number(choice)) {
      ...
      case 3:
        ...
        break;
      case 4:       
        const uid = await cin.question("Enter product ID to update: ");
        const newQty = await cin.question("Enter new quantity: ");
        await updateQuantity(Number(uid), Number(newQty));
        break;
      case 5:
